...
const rootPath = "/cloud/serverless";
const outputDir = new URL("../tmp/legacy-cloud-serverless/", import.meta.url);
...
async function crawl() {
  ...
  const allowedPrefix = new URL(rootPath, baseUrl).href;
  ...
  while (queue.length > 0) {
    ...
    for (const link of links) {
      ...
      if (!normalized.href.startsWith(allowedPrefix)) {
        continue;
      }
