...
Please read more about the [system requirements here.](/platform/guides/upgrade-to-litium-8/upgrade-the-platform-solution/dotnet-upgrade)
...
services:
  dnsresolver:
    ...
    container\_name: dnsresolver
    ports:
    - "127.0.0.1:53:53/tcp"
    - "127.0.0.1:53:53/udp"
    environment: 
    - DNS\_CNAME=\*.localtest.me=host.docker.internal
    - DNS\_FORWARDER=192.168.65.7
    dns: 192.168.65.7
  ...
  synonymserver:
    ...
    container\_name: synonymserver
    restart: unless-stopped
    ports:
    - "9210:80"
    environment:
    - DataFolder=/app\_data
    volumes:
    - ./volumes/synonymserver/data:/app\_data
  ...
  sqlserver:
    ...
    entrypoint: 
      ...
      # by using the specific permissions\_check entrypoint you assert that permissions are set on every restart
      - /bin/sh
      - -c
      - "/opt/mssql/bin/permissions\_check.sh && /opt/mssql/bin/sqlservr"
