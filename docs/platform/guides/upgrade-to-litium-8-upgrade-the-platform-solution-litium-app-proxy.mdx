...
The list of URLs is configured in Litium.Accelerator.MVC - `appsettings.json` under the `Litium` node. It is a dictionary where the key is the notification URL and the value is a destination URL on the app.
...
In a PSP add-on, the format of the endpoint used to received notification from payment provider is `https://<domain-name>/PaymentProviderResult.axd/Report/<payment-provider-name>`.
...
2.  The [push notification URL as defined here](/apps/klarna-payment/overview). In the code example below, the URL is /site.axd/KlarnaPayment/PushNotification.
...
Please note that App proxy-middleware is already installed per default in the [Litium accelerator](/accelerators/mvc/overview). 
