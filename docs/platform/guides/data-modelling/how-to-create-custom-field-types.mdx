...
description: "Learn how to create a custom field type that will be rendered in the backoffice UI"
---
...
For example, the field type can have as settings that it is possible to show the field in a grid - field type level setting - and in field definition we also have the same setting. The combination of the field type and field definition setting will define how that field behave. Let's see this chart:

- Field Type setting: allow in grid, Field definition setting: allow in grid → The field instance will be allowed in grid.
...
- Field type setting: not allow in grid, Field definition setting: allow in grid → The field instance will not be allowed in grid.
...
namespace Litium.Accelerator.FieldTypes
{
    public class CustomTextFieldTypeMetadata : FieldTypeMetadataBase
    {
        ...
        public override bool CanBeSortableField => false;
        public override bool IsArray => false;
        public override Type JsonType => typeof(string);

        protected override IFieldTypeConverter CreateConverter()
        {
            return new Converter();
        }

        private class Converter : IFieldTypeConverter
        {
            public object GetValue(ICollection<FieldData> fieldDatas) => fieldDatas.FirstOrDefault()?.TextValue;

            public ICollection<FieldData> PersistFieldData(object item) => PersistFieldDataInternal(item);

            protected ICollection<FieldData> PersistFieldDataInternal(object item) => new[] { new FieldData { TextValue = (string)item } };
        }
    }
}
```
...
namespace Litium.Accelerator.FieldTypes
{
    ...
    {
        private readonly FieldTypeMetadata _fieldTypeMetadata;
...
namespace Litium.Web.Administration.FieldFramework.Excel
{
    ...
    internal class CustomTextExcelFieldTypeConverter : IExcelFieldTypeConverter
    {
        ...
        public object ConvertToExcelValue(ExcelFieldTypeConverterArgs args, object item) => item;
    }
}
```
...
You can read more about the [template syntax in Angular here](https://angular.io/guide/template-syntax).
