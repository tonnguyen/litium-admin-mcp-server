...
This example uses Litium MVC Accelerator 8.17.3 version. It does not assume a particular payment provider app, but the code is tested using the [Qliro payment app](/apps/qliro-payment/overview).
...
Note that, to call a Web API method, you need to create a service account in Backoffice and [use it for authentication as shown here >>](/apis/overview)
