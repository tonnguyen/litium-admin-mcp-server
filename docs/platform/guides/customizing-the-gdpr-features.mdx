...
Define the custom data to be exported (or deleted) as a data record by creating a class according to the pattern in the class diagram below. _<T>_ in _DataRecord<T>_ is the class that defines the custom data (_ErpCustomerData_ in this example, see step 4).

![Customising-the-GDPR-features1-2.png](./media/Customising-the-GDPR-features1-2.png)

_DataRecord<T>_ has a parameter called _Data_, that needs to be populated to be accessed.
...
In this class you define what data is included in the data record that has been specified as custom data. In other words, this class defines the _<T>_ in _DataRecord<T>_.
