...
Local environment - A [Docker compose template](/add-ons/litium-apps/klarna-payment/docker-template) is available for local installation.
...
These keys are translated in Litium BO. [Learn how to use website text](/platform/areas/websites-cms/create-a-web).

The order validation callback will trigger the [Pre-order validation](/platform/areas/sales/order-placement/preorder-validation) in Litium.
