...
Local environment - A [Docker compose template](/add-ons/litium-apps/qliro-payment/docker-template) is available for local installation.
...
To use a saved Payment token, use the payment method "Payment Token" when creating the order. You need to set the actual payment token, for more information see documentation on [Payment Tokens](/platform/apis/connect-apis/domain-areas/payments-domain/business-processes/payment-tokens).
