...
The SupportedOperations metadata property lets Litium know which actions this app can perform. The possible values for the supported operations property is defined in the Connect APIs. See for example the supported operations of the [Connect Payments API](/api-reference/api?product=Litium.Connect.Payments&version=1.1&xref=Payments.#Litium_Connect_Payments_PaymentOperations).
