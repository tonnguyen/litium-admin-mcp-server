---
title: "Create Nuxt.js artifact"
description: "Create Nuxt.js artifact"
---
# Create Nuxt.js artifact

An artifact of type nuxtis used when you want to deploy a nuxtjs-web application into Litium Cloud.  
...
- Nuxt 3 - artifact for nuxt.js is building Nuxt version 3, i.e. other versions is not supported

## Preparing Nuxt.js package

### Access to packages

Nuxt.js application is built in Litium Serverless Cloud from the source, that means that it’s important that every resource that is used is public accessible. The npmrc file is used to configure custom npm registries for private packages, authentication information needs to be included with information about the private registries.
...
### Logging - Litium Insights logs from Nuxt.js

For Nuxt.js you need to install and configure the logging by yourself in your project to be able to view the logs in Litium Insights. [Read more about how to setup logging and see examples.](/cloud/serverless/artifacts-logging-for-nodejs-and-nuxtjs)
...
**_Note. Creating a Nuxt.js image may take some time, depending on project dependencies, size etc., because the build is happening in Serverless Cloud compared to dotnet where build is done before it is uploaded to Serverless cloud._**

The option "artifact-type" should be set to "nuxtjs" for nuxt.js applications

Example command could look like:

```powershell
litium-cloud artifact create --file-path .\frontend\ --artifact-type nuxtjs --subscription enter-subscription
```
...
##   
Summary - Steps to Create a Nuxt.js Artifact
...
3.  **Create the Nuxt.js Artifact Using Litium Cloud CLI**
