...
- Litium Platform - using requested backup of files (media) and database backup
    - Note. if you are upgrading from an earlier version than 8 you may need to handle [database upgrade manually](/platform/guides/upgrade-to-litium-8/upgrade-the-database) locally before uploading database-artifact
...
- Request production database backup and upload as artifact to Serverless Cloud for the correct subscription. _Note. schedule this with Litium Support in advance._
    - _​_If the current solution is Litium 8 or later database artifact may be uploaded directly to Serverless Cloud by Litium Support to save some time. _Important. make sure to schedule this as well in advance._
